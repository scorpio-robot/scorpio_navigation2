...
namespace scorpio_nav2_behaviors
{
  ...
  const std::shared_ptr<const BackUpAction::Goal> command)
{
  while (!costmap_client_->wait_for_service(std::chrono::seconds(1))) {
    if (!rclcpp::ok()) {
      ...
      return {nav2_behaviors::Status::FAILED, BackUpAction::Result::UNKNOWN};
    }
  ...
  if (result.wait_for(std::chrono::seconds(1)) == std::future_status::timeout) {
    RCLCPP_ERROR(logger_, "Interrupted while waiting for the service. Exiting.");
    return {nav2_behaviors::Status::FAILED, BackUpAction::Result::TIMEOUT};
  }
  ...
  if (!nav2_util::getCurrentPose(
    ...
    return {nav2_behaviors::Status::FAILED, BackUpAction::Result::TF_ERROR};
  }
  ...
  return {nav2_behaviors::Status::SUCCEEDED, BackUpAction::Result::NONE};
}
...
nav2_behaviors::ResultStatus BackUpFreeSpace::onCycleUpdate()
{
  ...
  if (!isCollisionFree(distance, cmd_vel->twist, pose)) {
    ...
    return {nav2_behaviors::Status::FAILED, BackUpAction::Result::COLLISION_AHEAD};
  }

  vel_pub_->publish(std::move(cmd_vel));

  return {nav2_behaviors::Status::RUNNING, BackUpAction::Result::NONE};
}
...
PLUGINLIB_EXPORT_CLASS(scorpio_nav2_behaviors::BackUpFreeSpace, nav2_core::Behavior)
